...
namespace std_dsp {
	...
	namespace generator_functors {
		...
		struct constant_generator_op {
			...
			constant_generator_op(scalar_t s) : s(s), v(load2(s)) {}
		...
		namespace detail {
			...
			inline
			double2_t uniform2() {
				return load2(uniform1(), uniform1());
			}
		}
		...
		struct random_generator_op {
			...
			random_generator_op(scalar_t a, scalar_t b) : offset(a), scale(b-a), offset_v(load2(a)), scale_v(load2(scale)) {}
			...
			inline
			double2_t get2() { return add(offset_v, multiply(scale_v, detail::uniform2())); }			
		};
	}
	...
	namespace transform_functors {
		struct identity_op {
			...
			inline
			double2_t operator()(double2_t x) { return x; }
		};
		...
		struct clip_op {
			scalar_t min_level;
			scalar_t max_level;
			double2_t min_level_v;
			double2_t max_level_v;

			clip_op(scalar_t min_level, scalar_t max_level) : min_level(min_level), max_level(max_level) {
				min_level_v = load2(min_level, min_level);
				max_level_v = load2(max_level, max_level);
			}
		...
		struct cubic_clip_op {
			...
			double2_t one_third_v;
			double2_t three_halves_v;
			double2_t max_value_v;
			double2_t min_value_v;
			double2_t pre_gain_v;

			cubic_clip_op() {
				one_third = 1.0 / 3.0;
				three_halves = 3.0 / 2.0;
				one_third_v = load2(one_third, one_third);
				three_halves_v = load2(three_halves, three_halves);
				max_value_v = load2(1.0, 1.0);
				min_value_v = load2(-1.0, -1.0);
				pre_gain_v = load2(0.707945784384138, 0.707945784384138);
			}
	...
	namespace reduction_functors {
		struct min_value_op {
			...
			min_value_op() : s(0.0), v(load2(0.0, 0.0)) {}
		...
		struct abs_op {
			...
			abs_op(Op op) : inner_op(op), sign_mask(load2(-0.0)) {
			}
